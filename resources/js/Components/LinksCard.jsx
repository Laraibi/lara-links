...
export default function LinksCard({ links: initialLinks = [] }) {
    ...
    const handleDelete = (link) => {
        if (confirm(t('links.confirmDelete'))) {
            destroy(route('links.destroy', link), {
                ...
                onSuccess: () => {
                    ...
                    setNotification({
                        type: 'success',
                        message: t('links.deleteSuccess')
                    });
                },
                onError: () => {
                    setNotification({
                        type: 'error',
                        message: t('links.deleteError')
                    });
                }
            });
        }
    ...
    const handleUpdate = (link) => {
        ...
        updateForm.put(route('links.update', link), {
            ...
            onSuccess: () => {
                ...
                setNotification({
                    type: 'success',
                    message: t('links.updateSuccess')
                });
            },
            onError: () => {
                setNotification({
                    type: 'error',
                    message: t('links.updateError')
                });
            }
        });
    };
                                ...
                                <div key={link.id} className="bg-white border border-gray-200 rounded-lg shadow-sm p-4">
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1 min-w-0">
                                            {editingLinkId === link.id ? (
                                                <input
                                                    ...
                                                    placeholder={t('links.enterLinkName')}
                                                />
                                            ) : (
                                                <h3 className="text-lg font-medium text-gray-900 truncate">
                                                    {link.name || t('links.unnamedLink')}
                                                </h3>
                                            )}
                                        ...
                                        <div className="ml-4 flex-shrink-0 flex space-x-2">
                                            ...
                                            <button
                                                ...
                                                className="text-gray-400 hover:text-gray-500"
                                                title={link.qr_code_path ? t('links.viewQrCode') : t('links.generateQrCode')}
                                            >
                                    ...
                                    <div className="mt-4 flex items-center justify-between">
                                        <div className="flex items-center text-sm text-gray-500">
                                            <span>{link.visits_count} {t('links.visits')}</span>
                                            <span className="mx-2">•</span>
                                            <span>{t('links.created')} {new Date(link.created_at).toLocaleDateString()}</span>
                                        </div>
                                        {editingLinkId === link.id && (
